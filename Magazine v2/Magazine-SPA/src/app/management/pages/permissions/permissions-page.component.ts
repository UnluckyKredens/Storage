...
import { PermissionModalComponent } from '../../modals/permission/permission-modal.component';
...
export class PermissionsPageComponent implements OnInit, AfterViewInit {
  ...
  showDetails(row: Permission): void {
  ...
  add(): void {
    ...
    this.openModal('form', null)
      ...
      .subscribe((saved) => {
        if (!saved) return;
        this.notification.success('Uprawnienie dodane.');
        this.load();
      });
  }
  ...
  edit(row: Permission): void {
    ...
    this.openModal('form', row)
      ...
      .subscribe((saved) => {
        if (!saved) return;
        this.notification.success('Uprawnienie zostało zaktualizowane.');
        this.load();
      });
  }

  remove(row: Permission): void {
    if (!this.canManage) return;
    this.openModal('delete', row)
      ...
      .subscribe((confirmed) => {
        ...
        this.service.delete(row.id).subscribe({
          next: () => {
            this.notification.success('Uprawnienie usunięte.');
            this.load();
          },
          error: (error: HttpErrorResponse) => this.showError(error),
        });
  ...
  private openModal(mode: 'details' | 'form' | 'delete', row: Permission | null) {
    let width = '720px';
    if (mode === 'details') width = '640px';
    if (mode === 'delete') width = '480px';

    return this.dialog.open(PermissionModalComponent, {
      width,
      maxWidth: '95vw',
      data: { mode, row },
    });
  ...
  private showError(error: HttpErrorResponse): void {
    this.notification.error(error.error?.message ?? 'Nie udało się pobrać uprawnień.');
    this.loading = false;
  }
}
