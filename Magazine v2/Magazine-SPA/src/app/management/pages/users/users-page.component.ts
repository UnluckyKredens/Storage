...
import { User } from '../../models/management.models';
...
export class UsersPageComponent implements OnInit, AfterViewInit {
  ...
  get canManage(): boolean {
    return (
      ...
      this.account.permissionCodes().includes('users.manage')
    );
  }
  ...
  add(): void {
    ...
    this.openModal('form', null)
      ...
      .subscribe((saved) => {
        if (!saved) return;
        this.notification.success('Użytkownik dodany.');
        this.load();
      });
  }
  ...
  edit(row: User): void {
    ...
    this.openModal('form', row)
      ...
      .subscribe((saved) => {
        if (!saved) return;
        this.notification.success('Użytkownik został zaktualizowany.');
        this.load();
      });
  }
    ...
    this.openModal('delete', row)
      ...
      .subscribe((confirmed) => {
        ...
        this.service.delete(row.id).subscribe({
          next: () => {
            this.notification.success('Użytkownik usunięty.');
            this.load();
          },
          error: (error: HttpErrorResponse) => this.showError(error),
        });
  ...
  private openModal(mode: 'details' | 'form' | 'delete', row: User | null) {
    let width = '720px';
  ...
  private showError(error: HttpErrorResponse): void {
    this.notification.error(error.error?.message ?? 'Nie udało się pobrać użytkowników.');
    this.loading = false;
  }
}
